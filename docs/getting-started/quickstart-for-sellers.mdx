...
**`batch-settlement`** — For **high-frequency** or **micropayment** traffic on **EVM**, the buyer **funds a channel once**, signs **off-chain vouchers** per request, and the seller **settles onchain in batches** (not every request). Each call still advertises a **per-request maximum** (`price`); you can charge **actual usage** up to that cap using the same **`setSettlementOverrides`** pattern as `upto`. Reference implementations: **TypeScript** and **Go**; Python is not available yet. See **[Batch settlement](/core-concepts/batch-settlement)** and the [examples in the repo](https://github.com/x402-foundation/x402/tree/main/examples/typescript/servers/batch-settlement).
