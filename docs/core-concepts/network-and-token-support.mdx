...
For **high-frequency** or **micropayment** flows where settling **every** request onchain is too costly, x402 supports the **`batch-settlement`** scheme on EVM: the buyer **deposits once** into escrow, signs **off-chain vouchers** per request, and sellers **redeem in batches** onchain. Per-request pricing still uses an **up-to** ceiling (`PaymentRequirements.amount`); actual charges can vary per request within that limit—see **[Batch settlement](/core-concepts/batch-settlement)**.
